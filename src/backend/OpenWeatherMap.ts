...
import { Condition, PressureUnit } from '../types';
import Backend from './Backend';
import type { BackendOptions } from './Backend';
import type { Response } from 'express';

export default class OpenWeatherMap extends Backend {
  /**
   ...
   * @returns {string}
   */
  static #conditionCode(conditionCode: number) {
   ...
   * @returns {number|null}
   */
  static #volumeMm(volume: number | { '1h'?: number } | null | undefined) {
    if (volume == null) {
      return null;
    }
    if (typeof volume === 'object') {
      return volume['1h'] == null ? null : Number.parseFloat(String(volume['1h']));
    }
    return Number.parseFloat(String(volume));
  }
  ...
  serialize(data: Record<string, any>, unit: string): any {
    return {
      ...
      daily: data.daily.map((day: Record<string, any>) => {
        const {
          clouds,
          dew_point: dewPoint,
          dt,
        ...
        return {
          ...
          pressure: { value: pressure, unit: PressureUnit.MB },
      ...
      hourly: data.hourly.map((hour: Record<string, any>) => {
        ...
        return {
          ...
          pressure: { value: hour.pressure, unit: PressureUnit.MB },
